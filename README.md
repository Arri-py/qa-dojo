...
Приложение теперь читает настройки подключения к базе из переменных окружения:
...
```
docker-compose down → docker-compose up → GET /api/v1/calculations → данные на месте
```
